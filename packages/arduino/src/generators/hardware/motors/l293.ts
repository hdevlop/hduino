...
Arduino.forBlock['l293_init'] = function (block: Blockly.Block): string {
  const initBlock = block as L293InitBlock;
  ...
  Arduino.reservePin(block, pin0, PinTypes.OUTPUT, 'L293 Motor A1');
  Arduino.reservePin(block, pin1, PinTypes.OUTPUT, 'L293 Motor A2');
  if (pinENA) {
    Arduino.reservePin(block, pinENA, PinTypes.PWM, 'L293 Motor A EN');
  }
  ...
  if (motorCount === 2) {
    ...
    Arduino.reservePin(block, pin2, PinTypes.OUTPUT, 'L293 Motor B1');
    Arduino.reservePin(block, pin3, PinTypes.OUTPUT, 'L293 Motor B2');
    if (pinENB) {
      Arduino.reservePin(block, pinENB, PinTypes.PWM, 'L293 Motor B EN');
    }
...
function generateMovementFunctions(
  ...
  if (motorCount === 2) {
    ...
    leftCode += `  digitalWrite(${varName}A1, HIGH);\n  digitalWrite(${varName}A2, LOW);\n`;
    if (motorAHasEN && pinENA) {
      leftCode += `  analogWrite(${varName}ENA, ${varName}Speed);\n`;
    }
    leftCode += `  digitalWrite(${varName}B1, LOW);\n  digitalWrite(${varName}B2, HIGH);\n`;
    if (motorBHasEN && pinENB) {
    ...
    rightCode += `  digitalWrite(${varName}A1, LOW);\n  digitalWrite(${varName}A2, HIGH);\n`;
    if (motorAHasEN && pinENA) {
      rightCode += `  analogWrite(${varName}ENA, ${varName}Speed);\n`;
    }
    rightCode += `  digitalWrite(${varName}B1, HIGH);\n  digitalWrite(${varName}B2, LOW);\n`;
    if (motorBHasEN && pinENB) {
...
Arduino.forBlock['l293_movement'] = function (block: Blockly.Block): string {
  const varName = block.getField('VAR')?.getText() || 'MT';
  const direction = block.getFieldValue('DIRECTION') || 'Forward';

  block.setWarningText(null, 'L293_Movement');
  return `${varName}_${direction}();\n`;
};

Arduino.forBlock['l293_set_speed'] = function (block: Blockly.Block): string {
  const varName = block.getField('VAR')?.getText() || 'MT';
  ...
  if (initBlock) {
    const motorAHasEN = (initBlock as L293InitBlock).motorAHasEN_ ?? true;
    const motorBHasEN = (initBlock as L293InitBlock).motorBHasEN_ ?? true;
    const motorCount = (initBlock as L293InitBlock).motorCount_ ?? 2;
  ...
  block.setWarningText(null, 'L293_NoEN');
